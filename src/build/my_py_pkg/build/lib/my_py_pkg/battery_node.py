...
class BatteryNode(Node): # MODIFY NAME
    def __init__(self):
        ...
        #need to check time since last check and state of battery
    ...
    def call_set_led_server(self, led_number, state): #function to create client, create request, call server and wait for response 
        ...
        future.add_done_callback(partial(self.callback_call_set_led, led_number=led_number, state=state)) #adds a future callback function for when the server sends a response



    def callback_call_set_led(self, future, led_number, state): #send parameters so we know whc=ich response is for which request
        try:
